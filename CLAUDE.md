...
- **Check CodeQL before merging code.** CodeQL is advisory, not a gate (see Git workflow) — a real alert will **not** block the merge for you. So on any PR that touches code — *especially* the security invariants above — review its CodeQL findings before merging: the PR's `CodeQL` check annotations on the diff, or the repo's **Security → Code scanning** tab. Workflow-, docs-, and dependency-only PRs have nothing for CodeQL to analyze, so there's nothing to check.
